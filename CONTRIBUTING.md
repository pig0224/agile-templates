...
2. 在 `registry.json` 的 `singles` 数组登记（条目 = `{ "name", "description", "language"?, "framework"? }`；无 path 字段，目录由名字派生）
3. 模板内容要求：
...
1. 为每个成员新建 `solutions/<组合名>/<成员名>/`——**组合专属完整模板骨架**（可复制最接近的单例模板作起点，按组合需求定制；仓库无对应技术栈单例的成员从零手写；成员与 singles 互不引用）
2. 在 `registry.json` 的 `solutions` 数组登记组合：`{ "name", "description", "projects": [成员条目...] }`——成员条目与 singles 同形状（`{ "name", "description" }`），数组顺序 = 生成顺序
