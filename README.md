...
[Agile CLI](https://github.com/pig0224/agile-cli) 的**项目模板源**：`registry.json` 声明全部模板。**新增模板无需升级 CLI** —— 新建目录 + 登记 registry.json 即可。更新推送后即时生效，无需 npm、无需构建。
...
**命名规范（防冲突）**：格式 `^[a-z][a-z0-9-]*$`；模板名 / 组合名 / 成员名三段全局唯一（init 后全部平铺落盘 `projects/`，同一命名空间）；目录名必须与 registry.json 中的 name 完全一致（无 path 字段，目录由名字派生；CLI 与 check 脚本双重强制校验，不一致直接拒绝）。命名模式：模板 `<技术栈/框架>-<变体>`（`vue3-vite`、`go-service`…）；组合 `<系统域>-<定位>`（`admin-base`…）。完整命名与一致性校验规则见 [docs/registry.md](./docs/registry.md)。
...
**缓存**：本仓库的本地缓存位于 `~/.agile/templates/<url-hash>`（用户级，跨 workspace 共享），默认读缓存，`agile template update` 时联网刷新，失联降级使用本地缓存。workspace 外执行 `agile template list` / `agile template update` 自动使用内置官方模板源。

完整规则：[docs/registry.md](./docs/registry.md)；开发约定见 [CLAUDE.md](./CLAUDE.md)。
