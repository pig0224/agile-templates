...
> 模板 react-vite 预填的默认值，项目演进时直接在本文件修订与追加。
...
├── stores/        # 状态管理（如引入 zustand）
└── api/           # 接口层（契约对齐任务 design.md）
```

## 命名约定

- 组件文件 PascalCase（如 `UserCard.tsx`）；Hook 文件 `useXxx.ts`
- 单测与被测文件同目录同名 `*.test.tsx`；e2e 固化脚本在 `e2e/*.spec.ts`
- 常量 UPPER_SNAKE_CASE；类型 / 接口 PascalCase；其余 camelCase

## 测试约定

- 单测 vitest：每个组件 / Hook 至少覆盖正常 + 边界路径（先写失败测试）
